...
The default serializer in _cod_ uses Ruby object marshaling as a form of on
the wire communication that is both efficient and fast. The downside is that
you cannot communicate with other versions of Ruby.
...
<pre class="sh_ruby"><code title="transmission of cod channels">
  ...
  read.get.object_id    # => 70245822725860
  read.object_id        # => 70245822725860
</code></pre>

h2. Implementing a serializer
...
transmits its messages as protobuf objects on the wire. The format chosen here
...
serializer. If you arrange your definitions so that you don't have to transmit
lengths and class name, communication can be very efficient.
...
There is a proof of concept implementation called
"panzer":https://github.com/kschiess/panzer. Using cod, it listens to HTTP
traffic. Inside a _cod_ serializer it turns that traffic into a
"rack":http://rack.github.com/ compatible request with the aid of
"net-http-server":github.com/postmodern/net-http-server. (yes. a web server.)
